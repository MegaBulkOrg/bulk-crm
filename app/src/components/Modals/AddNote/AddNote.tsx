...
export function AddNote({setNotifications}:TProps) {
    ...
    if (authState.authUserRoleId === 1 && location.pathname === '/') {
    ...
    const onSubmit: SubmitHandler<TForm> = async (data) => {        
        const id_deal = location.pathname === '/'
            ? data.deal?.value || 1
    ...
    return (
        <div className={styles.addNote} ref={modalRef}>
            <button className={styles.addNote__closeBtn} onClick={modalClose}>
                <FontAwesomeIcon icon={faClose} />
            </button>
            <h2 className={styles.addNote__title}>Добавление новой записи</h2>
            <form className={styles.addNote__form} autoComplete='on' onSubmit={handleSubmit(onSubmit)}>                                
                <InputField
                    title="Заголовок"
                    name="title"
                ...
                <RichTextField
                    ...
                    errors={errors}
                />
                <div className={styles.addNote__formBtns}>
                    <button className={styles.addNote__submitBtn} type='submit'>
                        Сохранить
